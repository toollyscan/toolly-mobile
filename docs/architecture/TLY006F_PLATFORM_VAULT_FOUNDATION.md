...
## Android Keystore IV contract

Wrapping keys retain `setRandomizedEncryptionRequired(true)`. Encryption initializes the Android
Keystore cipher without an IV, allows the provider to generate a random IV, and stores the returned
`Cipher.iv` in the authenticated envelope. Decryption supplies that stored IV. Toolly must not
disable randomized encryption or provide an IV when initiating encryption with a Keystore wrapping
key.

Content encryption uses a random per-record or per-asset data key outside Android Keystore and
...
device-specific vault defect.

This slice uses test-only generated bytes. It contains no hardcoded production content, user data,
